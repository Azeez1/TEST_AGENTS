...
hooks:
  - type: PreToolUse
    matcher: Edit
    hook_command: "powershell.exe -ExecutionPolicy Bypass -Command \"$input = [Console]::In.ReadToEnd(); $json = $input | ConvertFrom-Json; $path = $json.tool_input.file_path; if($path -notmatch 'FINANCIAL_TEAM'){ Write-Output ('{\\\"decision\\\":\\\"block\\\",\\\"reason\\\":\\\"BLOCKED by financial-guard: Edit outside FINANCIAL_TEAM/ directory. Financial agents must only write within FINANCIAL_TEAM/.\\\"}'); exit }; Write-Output '{\\\"decision\\\":\\\"approve\\\"}'\""
  - type: PreToolUse
    matcher: Write
    hook_command: "powershell.exe -ExecutionPolicy Bypass -Command \"$input = [Console]::In.ReadToEnd(); $json = $input | ConvertFrom-Json; $path = $json.tool_input.file_path; if($path -notmatch 'FINANCIAL_TEAM'){ Write-Output ('{\\\"decision\\\":\\\"block\\\",\\\"reason\\\":\\\"BLOCKED by financial-guard: Write outside FINANCIAL_TEAM/ directory. Financial agents must only write within FINANCIAL_TEAM/.\\\"}'); exit }; Write-Output '{\\\"decision\\\":\\\"approve\\\"}'\""
---
