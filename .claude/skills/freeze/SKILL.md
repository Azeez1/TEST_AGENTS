...
hooks:
  - type: PreToolUse
    matcher: Edit
    hook_command: "powershell.exe -ExecutionPolicy Bypass -Command \"$input = [Console]::In.ReadToEnd(); $json = $input | ConvertFrom-Json; $path = $json.tool_input.file_path; $configPath = Join-Path $PSScriptRoot 'config.json'; if(Test-Path $configPath){ $config = Get-Content $configPath -Raw | ConvertFrom-Json; $allowed = $config.allowed_directory; if($path -notmatch [regex]::Escape($allowed)){ Write-Output ('{\\\"decision\\\":\\\"block\\\",\\\"reason\\\":\\\"BLOCKED by freeze: Edit outside allowed directory: ' + $allowed + '\\\"}'); exit }}; Write-Output '{\\\"decision\\\":\\\"approve\\\"}'\""
  - type: PreToolUse
    matcher: Write
    hook_command: "powershell.exe -ExecutionPolicy Bypass -Command \"$input = [Console]::In.ReadToEnd(); $json = $input | ConvertFrom-Json; $path = $json.tool_input.file_path; $configPath = Join-Path $PSScriptRoot 'config.json'; if(Test-Path $configPath){ $config = Get-Content $configPath -Raw | ConvertFrom-Json; $allowed = $config.allowed_directory; if($path -notmatch [regex]::Escape($allowed)){ Write-Output ('{\\\"decision\\\":\\\"block\\\",\\\"reason\\\":\\\"BLOCKED by freeze: Write outside allowed directory: ' + $allowed + '\\\"}'); exit }}; Write-Output '{\\\"decision\\\":\\\"approve\\\"}'\""
---
